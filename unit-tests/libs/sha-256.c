...
static inline uint32_t right_rot(uint32_t value, unsigned int count) {
    /*
     * Defined behaviour in standard C for all count where 0 < count < 32, which is what we need
     * here.
     */
...
static inline void consume_chunk(uint32_t *h, const uint8_t *p) {
     ...
     *
     * This optimization was not there initially and the rest of the comments about w[64] are kept
...
void sha_256_init(struct Sha_256 *sha_256, uint8_t hash[SIZE_OF_SHA_256_HASH]) {
    ...
    sha_256->total_len = 0;
    /*
     * Initialize hash values (first 32 bits of the fractional parts of the square roots of the
     * first 8 primes 2..19):
     */
...
void sha_256_write(struct Sha_256 *sha_256, const void *data, size_t len) {
    ...
    while (len > 0) {
        /*
         * If the input chunks have sizes that are multiples of the calculation chunk size, no
         * copies are necessary. We operate directly on the input data instead.
         */
...
uint8_t *sha_256_close(struct Sha_256 *sha_256) {
    ...
    /*
     * The current chunk cannot be full. Otherwise, it would already have been consumed. I.e. there
     * is space left for at least one byte. The next step in the calculation is to add a single
     * one-bit to the data.
     */
    *pos++ = 0x80;
    --space_left;
