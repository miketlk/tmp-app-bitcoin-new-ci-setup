 ...
 */
static bool hash_update_reversed(cx_hash_t *hash_context, const void *in, size_t in_len) {
    const uint8_t *p_in = (const uint8_t *)in + in_len - 1;
    for (size_t i = 0; i < in_len; ++i) {
        if (!hash_update_u8(hash_context, *p_in--)) {
            return false;
        }
    }
    return true;
 ...
 * @return true on success, false in case of error.
 */
static bool generate_asset_entropy(const uint8_t contract_hash[static SHA256_LEN],
 ...
 * @return true on success, false in case of error.
 */
static bool compute_asset_tag_from_entropy(const uint8_t entropy[SHA256_LEN],
